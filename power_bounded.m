...
function [V,C,varargout] = power_bounded(x,y, wts, crs)
    ...
    % Allocate var to create polyshape object
    polyXY = cell(2,length(C));
    % Allocate var to determine adjacency
    AdjacencyXY = cell(length(C),1);
    for ij=1:length(C)
        % thanks to http://www.mathworks.com/matlabcentral/fileexchange/34428-voronoilimit
        Cij = C{ij};
        if (isempty(Cij))
            polyXY{1,ij} = V(C{ij},1);
            polyXY{2,ij} = V(C{ij},2);
            AdjacencyXY{ij} = V(C{ij},:);
            continue
        end
        ...
        if (all((X2 <= maxX) & (X2 >= minX) & (Y2 <= maxY) & (Y2 >= minY)))
            polyXY{1,ij} = V(C{ij},1);
            polyXY{2,ij} = V(C{ij},2);
            AdjacencyXY{ij} = V(C{ij},:);
            continue
        ...
        [xb, yb] = clip_polygons_orig(crs(:,1),crs(:,2),X2,Y2);
        % xb = xb'; yb = yb';
        ix=nan(1,length(xb));
        % iB = 1;
        for il=1:length(xb)
